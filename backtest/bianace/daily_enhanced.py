...
# 매매 전략 시뮬레이션
def simulate_moving_average_trading(df, window=40, initial_capital=500000, fee_rate=0.001):
    ...
    for i in range(1, len(df)):
        ...
        # 상승장에서 1만원 추가 매수 (40일 조건에 들지 않았을 때만)
        elif not in_position and df['Close'][i] > df['Close'][i-1] and df['Volume'][i] > df['Volume'][i-1]:
            # 추가 매수: 상승장(가격과 거래량이 증가)일 때 10,000원어치 매수
            additional_buy_price = df['Close'][i] * (1 + fee_rate)
            if capital >= 10000:
                additional_quantity = 10000 / additional_buy_price
                investment += 10000  # 추가 매수에 따른 투자금 증가
...
# 성과 지표 계산
def calculate_performance(trades, initial_capital=500000):
    if not trades:
        return {}

    profits = [trade[5] for trade in trades if trade[0] == 'sell']  # profit 인덱스 수정
...
initial_capital = 4000000  # 초기 자본금 40만원
