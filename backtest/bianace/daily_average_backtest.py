...
# 매매 전략 시뮬레이션
def simulate_trading(df, window, fee_rate=0.001):
    ...
    for i in range(1, len(df)):
        if df['Close'][i] > df[ma_column][i] and df['Close'][i-1] <= df[ma_column][i-1] and not in_position:
            ...
            in_position = True
            trades.append(('buy', df['Close Time'][i], buy_price))
            ...
            trades.append(('sell', df['Close Time'][i], sell_price, profit))
...
def calculate_performance(trades, window, initial_capital=10000):
    ...
    profits = [trade[3] for trade in trades if trade[0] == 'sell']
    ...
    for trade in trades:
        if trade[0] == 'buy':
            ...
            drawdown = (peak - trade[2]) / peak if peak != 0 else 0
...
def run_backtest(file_path, initial_capital):
    ...
    for window in [5, 10, 20, 50, 100, 200]:
        ...
        trades = simulate_trading(df, window)
